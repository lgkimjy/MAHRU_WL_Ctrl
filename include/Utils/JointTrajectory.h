...
private:
	int		TrajType;
	T dT, local_t, Time_End;
	T w_n;

	Eigen::Matrix<T, DIM, 1> c0, c1, c2, c3, c4, c5;
	Eigen::Matrix<T, DIM, 1> pos_init, pos_end;
	Eigen::Matrix<T, DIM, 1> vel_init, vel_end;
	Eigen::Matrix<T, DIM, 1> acc_init, acc_end;
...
public:
	...
	void setTargetPosition(const Eigen::Matrix<T, DIM, 1>& q_i, const Eigen::Matrix<T, DIM, 1>& q_f,
						const T& Time_Exec, const T& del_T, const int& trjType);

	void computeTraj(Eigen::Matrix<T, DIM, 1>& q_d, Eigen::Matrix<T, DIM, 1>& qdot_d, Eigen::Matrix<T, DIM, 1>& qddot_d);
};
...
////////////////////////////////////////////////////////////////////////////////
template<unsigned DIM, typename T>
inline void CP2P_Traj<DIM, T>:: setTargetPosition(const Matrix<T, DIM, 1>& q_i, const Matrix<T, DIM, 1>& q_f,
												const T& Time_Exec, const T& del_T, const int& trjType)
{
	local_t = 0;
...
template<unsigned DIM, typename T>
inline void CP2P_Traj<DIM, T>::computeTraj(Eigen::Matrix<T, DIM, 1>& q_d, Eigen::Matrix<T, DIM, 1>& qdot_d,
										Eigen::Matrix<T, DIM, 1>& qddot_d)
{
	if (local_t < Time_End) {
		local_t += dT;
