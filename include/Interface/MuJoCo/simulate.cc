...
void MakeUiSections(mj::Simulate* sim, const mjModel* m, const mjData* d) {
  ...
  MakeCommandSection(sim);  // JY CODE
}
...
void AlignAndScaleView(mj::Simulate* sim, const mjModel* m) {
  ...
  sim->cam.distance = 1 * m->stat.extent;
  ...
  // [sim.cam] elv :-25.747
  // [sim.cam] dist:2.607
  // [sim.cam] lookat:0.625
  // [sim.cam] lookat:0.090
  // [sim.cam] lookat:0.479
  // sim->cam.lookat[0] = 0.625;
  // sim->cam.lookat[1] = 0.090;
  // sim->cam.lookat[2] = 0.479;
  // sim->cam.azimuth = -140;
  // sim->cam.elevation = -24;
  // sim->cam.distance = 2.607;
