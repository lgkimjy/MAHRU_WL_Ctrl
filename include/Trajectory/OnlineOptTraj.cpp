...
OnlineOptTraj::~OnlineOptTraj() = default;

bool OnlineOptTraj::compute(
    ...
    const double& end_time)
{
    ...
    t_vec(mid_time, time_vec_z_);
    q_.tail(nb_var_z_) =
        -time_vec_z_ * mid_air_height_ * cost_z_;
    ...
    if (!qp_solver_.solve(Q_, q_, A_eq_, B_eq_, A_ineq_, B_ineq_))
    {
        ...
        {
            std::cout << "OnlineOptTraj::compute -> the "
                         "minimization "
                         "problem has no "
                         "solution! ("
                      << "start, current, end="
        ...
        {
            std::cout
                << "OnlineOptTraj::compute -> problems with "
                   "decomposing D!"
                << std::endl;
        }
        return false;
...
void OnlineOptTraj::get_next_state(
    ...
    Eigen::Ref<Eigen::Vector3d> next_acceleration)
{
    ...
    else
    {
        ...
        next_velocity = next_velocity / duration;   // Rescale to non-local time.
